...
@interface QCLineChartView()
@property (nonatomic,strong)QCLineDataSet * dataSet;
;
...
@end
@implementation QCLineChartView
...
- (void)setCurrentDataMaxAndMin
{
    ...
    if (self.dataSet.data.count > 0) {
        ...
        for (NSInteger i = idx; i < self.startDrawIndex + self.countOfshowCandle && i < self.dataSet.data.count; i++) {
            QCLineEntity  * entity = [self.dataSet.data objectAtIndex:i];
...
- (void)drawCandle:(CGContextRef)context
{
    ...
    for (NSInteger i = idex ; i< self.dataSet.data.count; i ++) {
        ...
        if (open < close) {
            ...
            if (i > 1) {
                QCLineEntity * lastEntity = [self.dataSet.data objectAtIndex:i-1];
