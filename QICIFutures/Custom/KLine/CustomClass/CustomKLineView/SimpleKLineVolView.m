...
- (void)p_drawText:(NSString *)text content:(CGContextRef)ctx textRect:(CGRect)rect {
    ...
    NSDictionary *attributes = @{
                                 ...
                                 NSForegroundColorAttributeName:QICIColorNormalText,   // 字体颜色
...
- (void)p_showDetailViewWithKLineModel:(KLineModel *)model isLeft:(BOOL)isLeft {
    ...
    // 时间
    DetailDataModel *timeModel = [[DetailDataModel alloc] initWithName:@"日期" desc:[NSString gl_convertTimeStamp:(model.stamp / 1000) toFormatter:@"yy-MM-dd hh:mm"]];
    // 开
...
- (ASDKLineView *)kLineMainView {
    if (!_kLineMainView) {
//        _kLineMainView = [[ASDKLineView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, (self.frame.size.height - 20.0f) * 7.0/10.0) config:self.mainViewConfig];
//        _kLineMainView.backgroundColor = ASDColorMarketDetail;
//        // 添加绘图算法
...
- (ASDKLineView *)volView {
    if (!_volView) {
//        _volView = [[ASDKLineView alloc] initWithFrame:CGRectMake(0, (CGRectGetMaxY(self.kLineMainView.frame) + 20.0f), self.frame.size.width, ((self.frame.size.height - 20.0f) * 3.0/10.0)) config:self.volViewConfig];
//        _volView.backgroundColor = ASDColorMarketDetail;
//        // 替换逻辑处理对象为主图逻辑处理对象
//        [_volView replaceDataLogicWithLogic:self.kLineMainView.dataLogic];
//
//        // 添加绘图算法
//        [_volView addDrawLogic:[[KLineMACDDrawLogic alloc] initWithDrawLogicIdentifier:@"macd"]];
...
//        [_verticalLineView setBackgroundColor:ASDColorTitle];
...
- (UIView *)horizontalLineView {
    if (!_horizontalLineView) {
        ...
        _horizontalLineView.backgroundColor = QICIColorTitle;
        _horizontalLineView.userInteractionEnabled = NO;
    }
