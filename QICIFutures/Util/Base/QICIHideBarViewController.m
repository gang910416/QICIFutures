...
@interface QICIHideBarViewController ()<UIGestureRecognizerDelegate>
...
@implementation QICIHideBarViewController
...
- (void)viewDidLoad {
    ...
    UIImage * img = [UIImage imageNamed:@"icon_ASD_1"];
...
//    navBar.backgroundColor = [UIColor whiteColor];
    navBar.backgroundColor = QICIColorTheme;
...
//    navTitleView.backgroundColor = [UIColor whiteColor];
        navTitleView.backgroundColor = QICIColorTheme;
...
- (void)configBaseLayout
{
    [self.navBar mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(QICIStatue_Height + 44);
    }];
...
- (void)configTitleViewLayout
{
    [self.navTitleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.left.bottom.mas_equalTo(self.navBar);
        make.top.mas_equalTo(self.navBar).mas_offset(QICIStatue_Height);
    }];
}
- (void)configTitleLayout
{
    CGFloat y = (44  + QICIStatue_Height) / 2 - 22;
    [self.navTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(kDeviceWidth - 100.0f);
...
{
    BOOL canBack = YES;
    if (self.navBackAction) {
        canBack = self.navBackAction(button);
    }
