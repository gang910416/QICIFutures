...
#import "BaseLoading.h"

static CGFloat ballScale = 1.5f;

@interface BaseLoading ()<CAAnimationDelegate>
{
...
+(void)showInView:(UIView *)view{
    ...
    BaseLoading *loading = [[BaseLoading alloc] initWithFrame:view.bounds];
    [view addSubview:loading];
    [loading start];
}

+(void)hideInView:(UIView *)view{
    for (BaseLoading *loading in view.subviews) {
        if ([loading isKindOfClass:[BaseLoading class]]) {
            [loading stop];
